// ASSESSMENT 1: Coding practical questions

// To run file: navigate to the correct directory in terminal, $ node code-challenges.js

// Please read all questions thoroughly!
// If you get stuck, please leave comments to help us understand your thought process.
// --------------------1) Create a statement that determines if a temperature is below boiling point, at boiling point, or above boiling point. Boiling point is 212 degrees Fahrenheit.

// Create the code and test each of the variables provided. Expected output: "42 is below boiling point", "350 is above boiling point", "212 is at boiling point"

var temp = [42, 350, 212]
for (i = 0; i < temp.length; i++) {
  if (temp[i] === 212){
    console.log(`${temp[i]} is at boiling point`);
  } else if (temp[i] < 212) {
    console.log(`${temp[i]} is below boiling point`);
  } else if (temp[i] > 212){
    console.log(`${temp[i]} is above boiling point`);
  }
}


// var temp = 350   var temp = 212



// --------------------2) Create the code that will combine the two arrays and return the length using the variables provided below. Expected output: 10

var myNumbers1 = [3, 7, 0, 36, -9]
var myNumbers2 = [8, -7, 42, 9, 13]
var combinedArrays = myNumbers1.concat(myNumbers2)
console.log(combinedArrays.length);


// --------------------3) Create the code that will reverse the letters of a string using the test variable provided below. Expected output: "1202 ohcE"

var currentCohort = "Echo 2021"
function reverseString (str) {
  let makeArray = str.split("")
  let reverseArray = makeArray.reverse()
  let reString = reverseArray.join("")
  console.log(reString);
}
reverseString(currentCohort)

// --------------------4) Create a statement that will evaluate whether each number in an array is even or odd. Use the test variable provided below. Expected output: "odd" "even" "odd" "even" "odd" "even"

var myArray = [13, 34, 5, 10, 27, 42]

//added an extra random number at the end
myArray.push(Math.floor(Math.random() * 1000000))

for (let i = 0; i < myArray.length; i++) {
    if (myArray[i] % 2 === 0) {
    console.log(`${myArray[i]} is even`);
  } else {
    console.log(`${myArray[i]} is odd`);
  }
}


// --------------------5) Create a statement that evaluates two numbers and subtracts the smaller number from the larger number using the two sets of test variables provided below. Expected output: 42, 3




function compareNumbers(number1, number2) {
   if (number1 === number2) {
    return console.log(`Both of the numbers are ${number1}`);
  } else if (number1 > number2) {
    let diffNums = (number1 - number2)
    return console.log(`${number1} is higher than ${number2} by ${diffNums}` );
  } else if (number1 < number2) {
    let diffNums = (number2 - number1)
    return console.log(`${number2} is higher than ${number1} by ${diffNums}` );
  }
}

var number1 = 58
var number2 = 100
compareNumbers(number1, number2)

number1 = 27
number2 = 24
compareNumbers(number1, number2)

//And here we have a random input to the function to have some variety every time, because variety is the spice of life.

number1 = Math.floor(Math.random() * 101)
number2 = Math.floor(Math.random() * 101)
console.log(number1);
console.log(number2);
compareNumbers(number1, number2)
